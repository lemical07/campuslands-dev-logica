...
- **Cantidad de victorias obtenidas** (número entero).
...
Si la cantidad de victorias es negativa, devuelve un mensaje indicando que el dato es inválido.
...
```javascript
{
    ...
    victorias: 12,
    clasificacion: "Clasificado a Playoffs"
}
```
...
```javascript
clasificarEquipo("Phoenix Team", -3);
```
...
```javascript
{
    ...
    mensaje: "La cantidad de victorias no puede ser negativa."
}
```